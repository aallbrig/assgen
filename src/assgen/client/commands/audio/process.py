...
def audio_trim_silence(
    ...
    threshold_db: float = typer.Option(
        -50.0, "--threshold-db", help="Silence threshold in dBFS (default -50)"
    ),
    output: str | None = _OUT_OPT,
    wait: bool | None = _WAIT_OPT,
) -> None:
...
def audio_convert(
    ...
    format: str = typer.Option("ogg", "--format", "-f", help="Target format: wav ogg mp3 flac"),
    output: str | None = _OUT_OPT,
    wait: bool | None = _WAIT_OPT,
) -> None:
    """Convert audio between formats (WAV/OGG/MP3/FLAC)."""
    submit_job(
        "audio.process.convert",
        {
            "input": input_file,
            "format": format,
            "output": output,
...
def audio_downmix(
    ...
    channels: int = typer.Option(
        1, "--channels", "-c", help="Target channel count: 1 (mono) or 2 (stereo)"
    ),
    output: str | None = _OUT_OPT,
    wait: bool | None = _WAIT_OPT,
) -> None:
