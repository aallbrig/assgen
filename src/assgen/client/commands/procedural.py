...
def terrain_heightmap(
    ...
    type: str = typer.Option("fractal", "--type", "-t", help="perlin | fractal | ridged"),
...
def texture_noise(
    width: int = typer.Option(512, "--width", "-W"),
    height: int = typer.Option(512, "--height", "-H"),
    noise_type: str = typer.Option("fbm", "--noise-type", "-n", help="perlin | voronoi | fbm"),
    seed: int = typer.Option(42, "--seed", "-s"),
    scale: float = typer.Option(100.0, "--scale"),
    octaves: int = typer.Option(6, "--octaves"),
    output: str | None = typer.Option(None, "--output", "-o"),
...
def level_dungeon(
    ...
    algorithm: str = typer.Option("bsp", "--algorithm", "-a", help="bsp | cellular"),
    seed: int = typer.Option(42, "--seed", "-s"),
    output: str | None = typer.Option(None, "--output", "-o"),
    wait: bool | None = typer.Option(None, "--wait/--no-wait"),
) -> None:
...
def plant_lsystem(
    ...
    rules: str = typer.Option(
        '{"F":"F[+F]F[-F]F"}', "--rules", "-r", help="JSON mapping symbol→replacement"
    ),
    iterations: int = typer.Option(4, "--iterations", "-i"),
    angle: float = typer.Option(25.0, "--angle", "-g", help="Turn angle in degrees"),
    step: float = typer.Option(10.0, "--step", "-s", help="Segment length in pixels"),
    output: str | None = typer.Option(None, "--output", "-o"),
    wait: bool | None = typer.Option(None, "--wait/--no-wait"),
) -> None:
    """Generate an L-system plant skeleton (SVG + branches JSON)."""
    submit_job(
        "procedural.plant.lsystem",
        {
            "axiom": axiom,
            "rules": rules,
