...
app = typer.Typer(
    help="HDRI skies, light probes, volumetrics, and lightmap baking.", no_args_is_help=True
)
...
def lighting_hdri(
    prompt: str = typer.Argument(
        ..., help="Sky / environment description, e.g. 'sunset desert sky'"
    ),
    resolution: int = typer.Option(
        4096, "--resolution", "-r", help="Output resolution (width of equirectangular image)"
    ),
...
def lighting_probes(
    ...
    probe_type: str = typer.Option("reflection", "--type", help="reflection | light | irradiance"),
    output: str | None = _OUT_OPT,
    wait: bool | None = _WAIT_OPT,
) -> None:
