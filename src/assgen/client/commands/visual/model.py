...
def model_create(
    ...
    input_image: str | None = typer.Option(
        None, "--input-image", "-i", help="Input image path or URL"
    ),
    format: str = typer.Option("glb", "--format", "-f", help="Output format: glb obj fbx"),
    target_faces: int | None = typer.Option(
        None, "--target-faces", help="Max triangles after decimation (default 10 000)"
    ),
...
def model_splat(
    ...
    convert_mesh: bool = typer.Option(
        False, "--convert-mesh", help="Also export a .ply alongside the .glb"
    ),
    target_faces: int = typer.Option(
        10_000, "--target-faces", help="Max triangles after trimesh decimation"
    ),
    output: str | None = _OUT_OPT,
    wait: bool | None = _WAIT_OPT,
) -> None:
...
def model_edit(
    ...
    secondary: str | None = typer.Option(
        None, "--secondary", help="Second mesh for boolean/combine"
    ),
    output: str | None = _OUT_OPT,
    wait: bool | None = _WAIT_OPT,
) -> None:
