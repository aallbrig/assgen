...
def run(job_type, params, model_id, model_path, device, progress_cb, output_dir):
    ...
    if not _AVAILABLE:
        raise RuntimeError("trimesh and numpy are required. Run: pip install trimesh numpy torch")
    ...
    if not raw_input or not input_path.is_file():
        ...
        candidate = next((f for f in upstream if Path(f).suffix.lower() in mesh_exts), None)
        if candidate:
...
def _rig_heuristic(mesh, use_humanoid: bool, progress_cb):
    ...
    parents = [names.index(parent_map[n]) if n in parent_map else -1 for n in names]
