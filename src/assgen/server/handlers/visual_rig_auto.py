...
Params:
    ...
    skeleton    (str):  biped | quadruped | custom (default: biped)
...
# Biped joint positions relative to mesh bounding-box centre (normalised 0-1)
_BIPED_JOINTS = {
    "Hips":          (0.5, 0.52, 0.5),
    "Spine":         (0.5, 0.60, 0.5),
    "Chest":         (0.5, 0.70, 0.5),
...
def run(job_type, params, model_id, model_path, device, progress_cb, output_dir):
    ...
    if _UNIRIG_AVAILABLE:
        ...
        result_scene = _rig_with_unirig(mesh, skeleton, device, model_path, progress_cb)
    else:
        progress_cb(0.15, "UniRig not installed — using heuristic biped placement…")
        result_scene = _rig_heuristic(mesh, skeleton, progress_cb)
...
def _rig_heuristic(mesh, skeleton, progress_cb):
    ...
    joint_template = _BIPED_JOINTS if skeleton == "biped" else _BIPED_JOINTS
    ...
    parents = _build_biped_parents(list(joints_world.keys()))
...
def _build_biped_parents(names: list[str]) -> list[int]:
    """Return parent index list for the canonical biped hierarchy."""
    _PARENT_MAP = {
    ...
    return [
        names.index(_PARENT_MAP[n]) if n in _PARENT_MAP else -1
        for n in names
    ]
...
def _build_skinned_scene(mesh, joints, parents, skin_weights):
    ...
    # Store rig metadata as scene metadata (parseable by downstream tools)
    scene.metadata = {
