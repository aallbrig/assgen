...
class TestValidateModelTaskCompatibilityIntegration:
    ...
    def test_hunyuan3d2_compatible_with_image_to_3d(self) -> None:
        ...
        ok, reason = validate_model_task_compatibility("tencent/Hunyuan3D-2", "image-to-3d", {})
        assert ok, reason
    ...
    def test_audiogen_medium_passes_validation_with_null_task(self) -> None:
        ...
        ok, reason = validate_model_task_compatibility("facebook/audiogen-medium", None, {})
        assert ok, reason
    ...
    def test_bark_compatible_with_text_to_speech(self) -> None:
        ...
        ok, reason = validate_model_task_compatibility("suno/bark", "text-to-speech", {})
        assert ok, reason
    ...
    def test_bark_incompatible_with_image_to_3d(self) -> None:
        ...
        ok, reason = validate_model_task_compatibility("suno/bark", "image-to-3d", {})
        assert not ok
...
class TestCatalogModelsValidateAgainstHF:
    ...
    def test_all_catalog_models_pass_validation(self) -> None:
        ...
        assert not failures, f"{len(failures)} catalog model(s) failed validation:\n" + "\n".join(
            failures
        )
